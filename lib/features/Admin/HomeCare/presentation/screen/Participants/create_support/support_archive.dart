...
class HomecareSupportsArchivePage extends StatefulWidget {
  const HomecareSupportsArchivePage({super.key});

  @override
  State<HomecareSupportsArchivePage> createState() => _SupportsArchivePageState();
}

class _SupportsArchivePageState extends State<HomecareSupportsArchivePage> {
  final List<SupportItem> _items = const [
    SupportItem(
      task: "Assistance with Daily Living",
      code: "1111",
      unit: r"$65 x 10 hours +",
      price: "3000.00",
    ),
    SupportItem(
      task: "Support 2",
  ...
  void _onRestore(SupportItem item) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Restored "${item.task}" (UI only)')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(title: "Supports", centerTitle: true),
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            children: [
              _PurpleHeaderBar(title: "Supports", onBack: _onBack),
              ...
              child: SingleChildScrollView(
                child: DataTable(
                  ...
                  rows: List.generate(items.length, (i) {
                    ...
                    return DataRow(
                      cells: [
                        DataCell(Text(e.task, style: rowStyle)),
                        DataCell(Text(e.code, style: rowStyle)),
                        DataCell(Text(e.unit, style: rowStyle)),
                        DataCell(Text(e.price, style: rowStyle)),
                        DataCell(
                          Wrap(
                            spacing: 10,
                            runSpacing: 10,
      ...
      itemBuilder: (context, i) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text(e.task, style: const TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 10),
              _kv("Code", e.code),
              _kv("Unit", e.unit, maxLines: 2),
              _kv("Price", e.price),
              const SizedBox(height: 10),
              Wrap(
                spacing: 10,
